...
const buildPostCubeDataInfo = function (
  headers,
  objectId,
  requestBody,
...
const buildGetCubeDataInfo = function (
  headers,
  objectId,
  instanceId,
  offset = DEFAULT_OFFSET,
...
const fetchJsonResultByUrl = function (baseUrl) {
  return async (request) => {
    const result = await fetch(baseUrl + request.url, request.requestInfo).then(
      (e) => e.json()
    );
    return result;
  };
...
const callREST = async function () {
  ...
  const publishCubeResult = await fetchFullResult(
    buildPublishCubeInfo(headerInfo, OBJECT_ID)
  );
  const cubePublishText =
    "publish cube result:" + "\n" + publishCubeResult.status;
  console.log(cubePublishText);
  fileContent.push(cubePublishText);

  const getCubeDefinitionResult = await fetchJsonResult(
    buildGetCubeDefinitionInfo(headerInfo, OBJECT_ID)
  );
  const cubeDefinitionText =
    "get cube definition result:" +
    "\n" +
    JSON.stringify(getCubeDefinitionResult);
  console.log(cubeDefinitionText);
  fileContent.push(cubeDefinitionText);

  const getCubeDataResult = await fetchJsonResult(
    buildPostCubeDataInfo(headerInfo, OBJECT_ID, "{}")
  );
  const cubeAllDataText =
    "get cube all data result:" + "\n" + JSON.stringify(getCubeDataResult);
  console.log(cubeAllDataText);
  fileContent.push(cubeAllDataText);

  const getRequestObjectsResult = await fetchJsonResult(
    buildPostCubeDataInfo(
      headerInfo,
      OBJECT_ID,
      JSON.stringify(REQUESTED_OBJECTS)
    )
  );
  const cubeRequsetObjectsText =
    "get cube data with request objects result:" +
    "\n" +
    JSON.stringify(getRequestObjectsResult);
  console.log(cubeRequsetObjectsText);
  fileContent.push(cubeRequsetObjectsText);

  const getSortingResult = await fetchJsonResult(
    buildPostCubeDataInfo(
      headerInfo,
      OBJECT_ID,
      JSON.stringify(Object.assign(REQUESTED_OBJECTS, SORTING))
    )
  );
  const cubeSortingText =
    "get cube data with request objects and sorting result:" +
    "\n" +
    JSON.stringify(getSortingResult);
  console.log(cubeSortingText);
  fileContent.push(cubeSortingText);

  const getMetricLimitsResult = await fetchJsonResult(
    buildPostCubeDataInfo(
      headerInfo,
      OBJECT_ID,
      JSON.stringify(Object.assign(REQUESTED_OBJECTS, SORTING, METRIC_LIMITS))
    )
  );
  const cubeMetricLimitsText =
    "get cube data with request objects, sorting and metric limits result:" +
    "\n" +
    JSON.stringify(getMetricLimitsResult);
  console.log(cubeMetricLimitsText);
  fileContent.push(cubeMetricLimitsText);

  const getViewFilterResult = await fetchJsonResult(
    buildPostCubeDataInfo(
      headerInfo,
      OBJECT_ID,
      JSON.stringify(
        Object.assign(REQUESTED_OBJECTS, SORTING, METRIC_LIMITS, VIEW_FILTER)
      )
    )
  );
  const cubeViewFilterText =
    "get cube data with request objects, sorting, metric limits and view filter result:" +
    "\n" +
  ...
  const getCubeInstanceDataResult = await fetchJsonResult(
    buildGetCubeDataInfo(headerInfo, OBJECT_ID, instanceId)
  );
  const cubeInstanceText =
    "get cube data with by cube instanc " +
    instanceId +
    " result:" +
    "\n" +
